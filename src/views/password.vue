    ...
    <el-container style="height: 100%; border: 1px solid #eee">
      ...
      <el-container style="height: 100%">
        <div style="
          ...
          <button v-on:click="changePassword()" style="right: -86%;position: relative;">修改密码</button>

          <button v-on:click="logout()" style="right: -86%;position: relative;">退出登录</button>
        </div>
        <el-header style="
            text-align: left;
            font-size: 12px;
        ...
        <el-main style="height: 100%; padding: 0px;">
          <el-form style="height:100%;background-color:aliceblue;" :model="ruleForm" status-icon :rules="rules"
            ref="ruleForm" label-width="100px" class="demo-ruleForm">
            <el-form-item label="原密码" prop="oldpass">
              <el-input type="password" v-model="ruleForm.oldpass" autocomplete="off" style="width:200px"></el-input>
            </el-form-item>
...
export default {
  ...
  methods: {
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          if(this.ruleForm.oldpass == this.oldPassword){
            var url = `http://localhost:8080/SSM_war_exploded/Huser/changePassword`;
          this.HuserForm.id = this.user.data.id;
          this.HuserForm.password = this.ruleForm.pass;
          axios
  ...
  created() {
    ...
    this.oldPassword = this.user.data.password;
    // this.selectallRoles();
  },
  mounted() {
