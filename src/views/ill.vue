<template>
  <span>
    <el-dialog id="dialog" title="疾病记录修改" :visible.sync="dialogFormVisible">
      <el-form :model="illForm" :rules="rules" ref="illForm">
        <el-form-item label="学号" prop="stuid">
          <el-input v-model="illForm.stuid" autocomplete="off" :disabled="true" ></el-input>
        </el-form-item>
        <el-form-item label="姓名" prop="username">
          <el-input v-model="illForm.username" autocomplete="off" :disabled="true" ></el-input>
        </el-form-item>
        <el-form-item label="日期" prop="date">
          <el-input v-model="illForm.date" autocomplete="off" ></el-input>
        </el-form-item>
        <el-form-item label="内容" prop="tittle">
          <el-input v-model="illForm.tittle" autocomplete="off" type="textarea"></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="submitForm('illForm')">确 定</el-button>
        <el-button @click="dialogFormVisible = false">取 消</el-button>
      </div>
    </el-dialog>

    <el-dialog id="showdialog" title="新增疾病记录" :visible.sync="dialogFormVisibleAdd">
      <el-form :model="illForm" :rules="rules" ref="illForm">
        <el-form-item label="学号" prop="stuid">
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="submitFormAdd('illForm')">确 定</el-button>
    ...
    <el-container style="height: 100%; border: 1px solid #eee">
      ...
      <el-container style="height: 100%">
        ...
        <el-header style="
            ...
            width: 100%;
            background-color: aliceblue;">
            <el-button @click="AddIll()">新增疾病记录</el-button>
          <span></span>
        </el-header>
  ...
  data() {
    var validatePass = (rule, value, callback) => {
      if (value === '') {
        ...
        if (this.illForm.checkPass !== '') {
          this.$refs.illForm.validateField('checkPass');
        }
        callback();
      }
    };
    var validatePass2 = (rule, value, callback) => {
      if (value === '') {
        callback(new Error('请输入密码'));
      } else if (value !== this.illForm.password) {
  ...
  methods: {
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          var url = `http://localhost:8080/SSM_war_exploded/Ill/updateIll`;
          axios
            .post(url, JSON.stringify(this.illForm), {
    ...
    submitFormAdd(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          var url = `http://localhost:8080/SSM_war_exploded/Ill/insertIll`;
          this.illForm.stuid = this.user.data.stuid;
          this.illForm.username = this.user.data.username;
          axios
            .post(url, JSON.stringify(this.illForm), {
              headers: { 'Content-Type': 'application/json' },
            })
    ...
    handleClick(row) {
      ...
      var url = `http://localhost:8080/SSM_war_exploded/Ill/selectIllById`;
      axios
        .get(url, {
          params: {
            id: row.id,
    ...
    handledelete(row) {
      var url = `http://localhost:8080/SSM_war_exploded/Ill/deleteIll`;
      axios
        .get(url, {
          params: {
            id: row.id,
    ...
    selectall() {
      var url = `http://localhost:8080/SSM_war_exploded/Ill/selectAll`;
